# OpenClaw 群晖部署

该部署与 Apex 容器独立，固定使用阿里云镜像仓库中的 OpenClaw
`2026.7.1-2`。Gateway 只发布到 NAS 回环地址，通过 SSH 隧道访问，不挂载
Docker socket。

## 1. 日常部署

登录 NAS 后，在任意目录执行：

```bash
deploy-openclaw.sh
```

该命令会依次拉取 Apex 最新代码、同步 Compose 和 Apex Skill、拉取固定镜像、
强制重建 Gateway、等待健康检查，并用“今天怎么操作？”执行一次只读契约探测。
探测日志只显示 `code`、`intent` 和 `dataLevel`，不会打印完整的持仓研判内容。

只检查配置、依赖和当前服务状态，不做部署：

```bash
deploy-openclaw.sh --check
```

脚本不会创建或覆盖 `/volume1/docker/openclaw/.env`，也不会删除 OpenClaw 的状态和
工作区数据。

## 2. 首次安装全局命令

Apex 代码已位于 `/volume1/docker/apex` 时，以普通 `Awe` 用户执行：

```bash
bash /volume1/docker/apex/integrations/openclaw/deployment/deploy-openclaw.sh \
  --install-command
```

安装过程使用 NAS 已授权的 `sudo -n /usr/local/bin/docker` 写入
`/usr/local/bin/deploy-openclaw.sh`，并在群晖默认 PATH 中安装
`/usr/bin/deploy-openclaw.sh` 入口，不需要进入 `sudo -i`。两个入口都只委托到 Apex
仓库中的脚本，因此后续代码更新后不会运行陈旧副本。安装器可以重复执行，但会拒绝
覆盖不属于 Apex 的同名命令。

## 3. 首次准备 OpenClaw 目录

当前 NAS 已完成此步骤。仅新 NAS 首次部署时需要创建目录和 `.env`：

```bash
ssh apex-nas
sudo mkdir -p /volume1/docker/openclaw/data/state
sudo mkdir -p /volume1/docker/openclaw/data/workspace/skills
sudo cp /volume1/docker/apex/integrations/openclaw/deployment/compose.yaml \
  /volume1/docker/openclaw/compose.yaml
sudo cp /volume1/docker/apex/integrations/openclaw/deployment/.env.example \
  /volume1/docker/openclaw/.env
sudo chown -R 1000:1000 /volume1/docker/openclaw/data
sudo chmod 600 /volume1/docker/openclaw/.env
sudo vim /volume1/docker/openclaw/.env
```

保存 `.env` 后安装全局命令，再继续完成下面的 Apex 配置和 OpenClaw 首次初始化。
初始化完成后，日常更新只需执行 `deploy-openclaw.sh`。

在 `.env` 中填写：

- `OPENCLAW_GATEWAY_TOKEN`：执行 `openssl rand -hex 32` 生成。
- `APEX_BOT_CLIENT_KEY`：与 Apex `.env.production` 中的值完全一致。
- `APEX_BOT_CLIENT_SECRET`：执行 `openssl rand -hex 32` 生成，并与 Apex
  `.env.production` 中的值完全一致。
- `APEX_BOT_EXTERNAL_USER_ID`：该客户端唯一授权的微信用户标识，与 Apex
  `.env.production` 中的值完全一致。
- `APEX_DOCKER_NETWORK`：Apex Compose 网络，默认 `apex_default`。
- `APEX_BOT_BASE_URL`：默认通过 Docker 网络直连
  `http://backend:8080`，不经过公网域名或 NAS Tailscale 地址。

不要把真实密钥提交到 Git，也不要在聊天或终端截图中展示。

## 4. 配置 Apex

编辑 Apex 已有的生产配置，不要从模板覆盖：

```bash
vim /volume1/docker/apex/.env.production
```

第一阶段只开启问答接口，主动微信推送保持关闭：

```dotenv
APEX_BOT_ENABLED=true
APEX_BOT_CLIENT_KEY=与OpenClaw一致
APEX_BOT_CLIENT_SECRET=与OpenClaw一致
APEX_BOT_APEX_USER_ID=该客户端唯一授权的Apex用户主键
APEX_BOT_EXTERNAL_USER_ID=该客户端唯一授权的微信用户标识
APEX_BOT_WECLAW_ENABLED=false
```

`APEX_BOT_APEX_USER_ID` 必须从 Apex 用户记录中确认，不能填写微信用户标识，也不能
根据组合名称猜测。`APEX_BOT_EXTERNAL_USER_ID` 必须来自实际微信 Channel 身份；任一
绑定缺失或请求身份不匹配时，Apex 会拒绝 Bot 请求。

之后由你按现有流程重建 Apex 后端。

## 5. OpenClaw 首次初始化

以下命令使用群晖 Container Manager 自带的 Docker：

```bash
cd /volume1/docker/openclaw
DOCKER=/usr/local/bin/docker
sudo -n $DOCKER compose --env-file .env -f compose.yaml pull
sudo -n $DOCKER compose --env-file .env -f compose.yaml run --rm --no-deps \
  --entrypoint node openclaw-gateway \
  dist/index.js onboard --mode local --no-install-daemon
sudo -n $DOCKER compose --env-file .env -f compose.yaml run --rm --no-deps \
  --entrypoint node openclaw-gateway \
  dist/index.js config set --batch-json \
  '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
sudo -n $DOCKER compose --env-file .env -f compose.yaml up -d openclaw-gateway
sudo -n $DOCKER compose --env-file .env -f compose.yaml ps
```

Onboarding 会要求选择模型供应商并填写对应 API Key。使用你已有的供应商即可；
该选择与 Apex 自己使用的 Kimi 配置相互独立。

默认镜像地址为：

```dotenv
OPENCLAW_IMAGE=registry.cn-hangzhou.aliyuncs.com/awe-images/openclaw:2026.7.1-2
```

这是固定版本标签，不要改成 `latest`。如果仓库改为私有，需要先在 NAS 执行
`docker login registry.cn-hangzhou.aliyuncs.com`，再执行拉取命令。

## 6. 从 Mac 打开控制台

保持下面的 SSH 隧道运行：

```bash
ssh -N -L 18789:127.0.0.1:18789 apex-nas
```

浏览器访问 `http://127.0.0.1:18789/`，填入 `.env` 中的 Gateway Token。

## 7. 手工验证 Apex Skill

只有排查一键探测失败时，才需要在 NAS 普通用户会话中手工执行：

```bash
cd /volume1/docker/openclaw
DOCKER=/usr/local/bin/docker
sudo -n $DOCKER compose --env-file .env -f compose.yaml run --rm openclaw-cli \
  agent --agent main --message "请问 Apex：宁德时代现在风险大吗？"
sudo -n $DOCKER compose --env-file .env -f compose.yaml run --rm openclaw-cli \
  agent --agent main --message "我今天亏多少"
sudo -n $DOCKER compose --env-file .env -f compose.yaml run --rm openclaw-cli \
  agent --agent main --message "今天怎么操作？"
```

官方镜像已包含 Skill 所需的 Node.js、`curl` 和 `openssl`。如果命令执行失败，先看
Gateway 日志和 Apex 返回的鉴权错误，不要在运行中的容器里临时安装软件。

重启后可确认新工具脚本已生效：

```bash
test -x /volume1/docker/openclaw/data/workspace/skills/apex-stock-assistant/scripts/apex_tool.sh
```

## 8. 微信通道边界

OpenClaw 和 Apex 问答验证通过后，再选择个人微信 Channel。不同插件的登录方式、
主动发送接口和收件人 ID 不兼容，因此当前不假设 WeClaw，也不启用 Apex 的
`/api/send` 客户端。确定实际微信插件后，再适配主动推送并配置真实收件人 ID。

## 9. 故障排查

```bash
cd /volume1/docker/openclaw
DOCKER=/usr/local/bin/docker
sudo -n $DOCKER compose --env-file .env -f compose.yaml ps
sudo -n $DOCKER compose --env-file .env -f compose.yaml logs --tail=200 openclaw-gateway
```

一键部署固定使用已确认的镜像版本，不使用 `latest`。状态和 Skill 均位于
`/volume1/docker/openclaw/data`，重建容器不会删除它们。
