...
readonly servlet_path="/apex/api/trade-events/ingest"
...
readonly canonical="$(printf '%s\n%s\n%s\n%s\n%s' "${http_method}" "${servlet_path}" "${timestamp}" "${nonce}" "${content_sha256}")"
readonly signature="$(printf '%s' "${canonical}" | openssl dgst -sha256 -hmac "${APEX_BOT_CLIENT_SECRET}" | awk '{print $NF}')"
...
curl --fail-with-body --silent --show-error \
    ...
    -- "${APEX_BOT_BASE_URL%/}${servlet_path}"
