...
test('mobile navigation drawer follows horizontal page swipes progressively', () => {
  ...
  assert.match(appSource, /:style="mobileMenuShellStyle"/)
  assert.match(appSource, /--mobile-menu-page-offset/)
  assert.doesNotMatch(appSource, /mobileMenuScrimStyle/)
  assert.match(appSource, /\.nav-scrim\s*\{[^}]*background:\s*transparent;/)
  assert.match(appSource, /const MOBILE_MENU_RIGHT_GAP = 16/)
  assert.match(appSource, /--mobile-menu-right-gap': `\$\{MOBILE_MENU_RIGHT_GAP\}px`/)
  assert.match(appSource, /mobileViewportWidth\.value - MOBILE_MENU_RIGHT_GAP/)
  assert.match(appSource, /\.links\s*\{[^}]*width:\s*calc\(100vw - var\(--mobile-menu-right-gap, 16px\)\);[^}]*max-width:\s*none;/)
  assert.match(appSource, /\.shell\s*\{[^}]*overflow-x:\s*clip;/)
  assert.match(appSource, /\.main\s*\{[^}]*transform:\s*translate3d\(var\(--mobile-menu-page-offset, 0px\), 0, 0\);/)
  assert.match(appSource, /\.links\.dragging\s*\{[^}]*transition:\s*none;/)
  assert.match(appSource, /\.mobile-menu-dragging \.main\s*\{[^}]*transition:\s*none;/)
})
