...
async function onCopyShare() {
  copying.value = true
  try {
    await copyImageBlob(captureObserveShare())
    ElMessage.success('已复制到剪贴板，可直接粘贴到微信/文档')
  } catch (e) {
    console.error('复制观察池分享图失败', e)
...
.search :deep(.el-input__wrapper) {
  min-height: 40px;
  padding: 1px 12px;
  border: 1px solid var(--glass-border, #dce3ed);
  border-radius: 8px;
  background: #fff;
