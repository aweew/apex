...
test('market board automatically follows the highest-priority active market', () => {
  ...
  assert.doesNotMatch(indexSource, /key: 'jp', label: '日本'|key: 'kr', label: '韩国'/)
  assert.match(indexSource, /asia: regionalMarketIndexes\.value\.jp\.concat\(regionalMarketIndexes\.value\.kr\)/)
  assert.match(indexSource, /async function syncActiveMarket\(\)[\s\S]*?resolveActiveMarket\(\)[\s\S]*?marketTab\.value !== nextMarketTab/)
