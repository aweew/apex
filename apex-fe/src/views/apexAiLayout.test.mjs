...
test('Apex AI restores conversations and enhances rule cards in place', () => {
  assert.match(source, /getLatestApexAiConversation/)
  assert.match(source, /enhanceXiaolingAnalysis/)
  assert.match(source, /conversationId\.value/)
  assert.match(source, /function startNewConversation\(\)/)
  assert.match(source, /enhancing:\s*context\.value\.aiConfigured/)
  assert.match(source, /message\.analysis = response\.data/)
  assert.match(source, /message\.enhanceFailed = true/)
  assert.match(source, /AI 增强暂不可用/)
