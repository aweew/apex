...
后端使用 Flyway 管理数据库版本。首次接管既有数据库时会登记版本 `43` 基线，并执行
`V44__Reconcile_market_schema`，一次性补齐此前由启动代码维护的表、字段、索引和历史数据。
迁移成功后会记录到 `flyway_schema_history`，后续启动只校验迁移历史，不再重复扫描或修改业务表。
迁移失败会直接阻止后端启动，部署脚本的健康检查也会失败并输出后端日志，不会让缺列或缺表的版本
以“健康”状态继续运行。`docs/sql` 中的独立脚本继续作为结构审计和手工排障依据，后续正式迁移从
版本 `45` 开始，正常更新部署无需手工重复执行。
